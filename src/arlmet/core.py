...
def _grid_to_attrs(grid: Grid) -> dict[str, Any]:
    attrs = {
        "arl_nx": grid.nx,
        "arl_ny": grid.ny,
    }
    ...
    attrs.update(
        {
            f"arl_{name}": getattr(projection, name)
            for name in PROJECTION_ATTRS
        }
    )
    return attrs


def _grid_from_attrs(attrs: Mapping[str, Any]) -> Grid:
    try:
...
def _vertical_axis_to_attrs(vertical_axis: VerticalAxis) -> dict[str, Any]:
    return {
...
def _vertical_axis_from_attrs(attrs: Mapping[str, Any]) -> VerticalAxis:
    try:
...
def _normalize_dataset_for_write(ds: xr.Dataset) -> xr.Dataset:
...
def _extract_dataset_source(attrs: Mapping[str, Any]) -> str:
...
def _extract_dataset_grid(attrs: Mapping[str, Any]) -> Grid:
    grid = attrs.get("grid")
    if isinstance(grid, Grid):
        return grid
    return _grid_from_attrs(attrs)


def _extract_dataset_vertical_axis(attrs: Mapping[str, Any]) -> VerticalAxis:
...
def _extract_dataset_forecasts(ds: xr.Dataset) -> np.ndarray:
...
def write_dataset(ds: xr.Dataset, filename_or_obj) -> None:
    ...
    ----------
    ds : xarray.Dataset
        Dataset following the native arlmet layout.
    filename_or_obj : str or Path
        Destination path for the ARL file.
    """
    ds = _normalize_dataset_for_write(ds)
    source = _extract_dataset_source(ds.attrs)
    grid = _extract_dataset_grid(ds.attrs)
    vertical_axis = _extract_dataset_vertical_axis(ds.attrs)
    forecasts = _extract_dataset_forecasts(ds)
    ...
    if height_coord is not None and height_coord.ndim == 1 and height_coord.dims == ("level",):
...
class RecordCollection(Mapping):
    ...
    def attrs(self) -> dict:
        ...
        attrs.update(_grid_to_attrs(self.grid))
        try:
        ...
        if vertical_axis is not None:
            ...
            attrs.update(_vertical_axis_to_attrs(vertical_axis))
        return attrs
    ...
    def to_xarray(self, drop_variables=None, squeeze=True) -> xr.Dataset | xr.DataArray:
        ...
        if self.records:
            forecast_by_time = self.forecast_by_time
            if "time" in ds.dims:
                ...
                ds = ds.assign_coords(forecast=only_forecast)
